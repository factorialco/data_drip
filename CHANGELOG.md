...
### Added
- **Multi-cell support** (opt-in): the same backfill or script can be triggered once and executed in every cell of a multi-deployment ("cell") architecture. The creating cell coordinates: it stores one dispatch record per target cell, delivers each over HTTPS to that cell's new machine-to-machine **Cell API** (`DataDrip::CellApi::Engine`, bearer-token auth, idempotent per `(group_uuid, cell_id)`), and its show page renders live per-cell cards (status, progress, errors, script output) fetched from each cell. Includes cell targeting on the new-run forms ("All cells / Only this cell / Choose cells…"), fan-out of stop/delete actions, per-cell retry of failed batches, a "Retry dispatch" flow for rejected deliveries, and origin badges in the run lists. Configured via `DataDrip.current_cell_id`, `cell_ids`, `cell_transport`, `cell_api_tokens`, and optional `cell_ui_url`; with the defaults nothing changes. Existing installs add the new columns and the `data_drip_cell_dispatches` table with `rails generate data_drip:add_multi_cell`.
